...
3.1. **Глобальный подвал (один раз на сайт или при обновлении ссылок):** выполни `python3 shared/deploy-footer.php` — заливает MU-plugin в `wp-content/mu-plugins/` (колонки Ресурсы/Контакты на всех страницах). Ссылки — `shared/nero-network-site-footer.php` и env `FOOTER_*`.
